...
def embed_text(req: TextEmbeddingRequest):
    ...
    try:
        with torch.no_grad(): # 不计算梯度，节省显存与算力
            ...
            # 【究极绝招】彻底放弃任何 get_text_features 辅助函数！
            # 直接调用 model.forward 通用前向传播。只传入 input_ids，让大模型只运行文本塔，
            # 获取官方标准定义的 outputs.text_embeds 纯 Tensor，100% 避开任何 API 封装结构转换 Bug！
            outputs = model(
...
async def embed_image(file: UploadFile = File(..., description="视频帧截图文件")):
    ...
    try:
        with torch.no_grad():
            ...
            # 【究极绝招】同样调用 model.forward，只传入 pixel_values 只跑视觉塔
            outputs = model(
