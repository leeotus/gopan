...
app = FastAPI(
    title="GoPan AI Vectorization Service - GPU Edition",
    description="本地离线多模态大模型向量化提取接口 (GPU 加速版、深度兼容)"
)
...
def load_clip_model():
    ...
    try:
        print("[GPU STARTUP] Loading CLIP model to GPU Video RAM ...")
        model = ChineseCLIPModel.from_pretrained(MODEL_NAME).to(device)
        processor = ChineseCLIPProcessor.from_pretrained(MODEL_NAME)
        model.eval()
        print("[Model Load] Model loaded successfully on NVIDIA GPU!")
