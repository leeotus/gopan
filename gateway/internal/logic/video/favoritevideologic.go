...
func (l *FavoriteVideoLogic) FavoriteVideo(r *http.Request) (resp *types.BaseResp, err error) {
	videoId, _ := strconv.ParseInt(r.URL.Query().Get("video_id"), 10, 64)
	userId := middleware.GetUserIdFromContext(l.ctx)

	_, rpcErr := l.svcCtx.InteractClient.Favorite(l.ctx, &interactclient.FavoriteReq{
		UserId:  userId,
		VideoId: videoId,
	})
	if rpcErr != nil {
